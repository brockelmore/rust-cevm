...
impl<'config> Gasometer<'config> {
    ...
    pub fn total_used_gas(&self) -> usize {
        match self.inner.as_ref() {
            Ok(inner) => {
                inner.used_gas + memory::memory_gas(inner.memory_cost).expect("Checked via record")
            }
    ...
    pub fn record_refund(&mut self, refund: isize) -> Result<(), ExitError> {
...
impl GasometerOwned {
    ...
    fn inner_mut(&mut self) -> Result<&mut InnerOwned, ExitError> {
        self.inner.as_mut().map_err(|e| *e)
    }
...
pub fn opcode_cost<H: Handler>(
    ...
    let gas_cost = match opcode {
        ...
        Err(ExternalOpcode::Address)
        | Err(ExternalOpcode::Origin)
        | Err(ExternalOpcode::Caller)
        | Err(ExternalOpcode::CallValue)
        | Ok(Opcode::CallDataSize)
        | Ok(Opcode::CodeSize)
        | Err(ExternalOpcode::GasPrice)
        | Err(ExternalOpcode::Coinbase)
        | Err(ExternalOpcode::Timestamp)
        | Err(ExternalOpcode::Number)
        | Err(ExternalOpcode::Difficulty)
        | Err(ExternalOpcode::GasLimit)
        | Ok(Opcode::Pop)
        | Ok(Opcode::PC)
        | Ok(Opcode::MSize)
        | Err(ExternalOpcode::Gas) => GasCost::Base,
        ...
        | Ok(Opcode::Xor)
        | Ok(Opcode::Byte)
        | Ok(Opcode::CallDataLoad)
        | Ok(Opcode::MLoad)
        | Ok(Opcode::MStore)
        | Ok(Opcode::MStore8)
        | Ok(Opcode::Push(_))
        | Ok(Opcode::Dup(_))
        | Ok(Opcode::Swap(_)) => GasCost::VeryLow,
        ...
        | Ok(Opcode::Mod)
        | Ok(Opcode::SMod)
        | Ok(Opcode::SignExtend) => GasCost::Low,
        ...
        Err(ExternalOpcode::ReturnDataSize) | Err(ExternalOpcode::ReturnDataCopy) => {
            GasCost::Invalid
        }
    ...
    let memory_cost = match opcode {
        ...
        Err(ExternalOpcode::Call) | Err(ExternalOpcode::CallCode) => Some(
            MemoryCost {
        ...
        Err(ExternalOpcode::DelegateCall) | Err(ExternalOpcode::StaticCall) => Some(
            MemoryCost {
...
impl<'config> Inner<'config> {
    fn memory_cost(&self, memory: MemoryCost) -> Result<usize, ExitError> {
    ...
    fn extra_check(&self, cost: GasCost, after_gas: usize) -> Result<(), ExitError> {
        match cost {
    ...
    fn gas_cost(&self, cost: GasCost, gas: usize) -> Result<usize, ExitError> {
        Ok(match cost {
            GasCost::Call {
                value,
                target_exists,
    ...
    fn gas_refund(&self, cost: GasCost) -> isize {
        match cost {
            GasCost::SStore {
                original,
                current,
